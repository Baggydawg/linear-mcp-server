...
async function main() {
  ...
  if (!apiKey) {
    console.error('ERROR: No API key found. Set LINEAR_ACCESS_TOKEN or PROVIDER_API_KEY in .env');
    process.exit(1);
  }
  ...
  for (const field of REQUIRED_FIELDS) {
    ...
    console.log(`${status} ${field.padEnd(15)} | client.users(): ${inUsers ? 'YES' : 'NO'} | team.members(): ${inMembers ? 'YES' : 'NO'}`);
  ...
  // 7. Show member count
  console.log(`Team "${team.name}" has ${membersConn.nodes.length} members (fetched up to 5)`);
