...
export const toolsMetadata = {
  ...
  list_projects: {
    ...
    title: 'List Projects',
    description:
      "List projects with filtering and pagination. Inputs: team? (team key like 'SQM' or UUID — overrides DEFAULT_TEAM), project? (short key like 'pr0'), filter? (ProjectFilter for advanced use), stateType?, priority?, includeArchived?, limit?, cursor?. For a single project, set filter.id.eq and limit=1.\n\nFILTERING (prefer dedicated params):\n- Active projects: stateType: 'started'\n- High priority: priority: 'high' (or 2)\n- Advanced: use filter for complex queries (date ranges, lead, etc.)\nReturns: { items[], cursor?, nextCursor?, limit? } where items include id, name, state, leadId?, teamId?, targetDate?, description?. Next: Use 'update_projects' to modify or 'list_issues' with projectId to find issues.\n\nTOON Output (when enabled): Tier 2 - includes only REFERENCED entities. Projects use short keys (pr0, pr1) in output.",
  },
