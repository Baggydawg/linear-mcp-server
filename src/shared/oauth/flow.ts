...
export async function handleAuthorize(
  ...
  if (providerConfig.clientId && providerConfig.clientSecret) {
    logger.info('oauth_authorize', {
      ...
      clientIdUsed: providerConfig.clientId.slice(0, 12) + '...',
      accountsUrl: providerConfig.accountsUrl,
    });

    // Ensure accountsUrl ends with / so 'authorize' appends correctly
    const baseUrl = providerConfig.accountsUrl.endsWith('/')
