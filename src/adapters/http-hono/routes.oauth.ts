...
export function buildOAuthRoutes(
  ...
  // DEBUG: Log provider config on startup
  logger.info('oauth_hono_init', {
    message: 'OAuth routes initialized',
    hasClientId: !!providerConfig.clientId,
    clientIdPrefix: providerConfig.clientId?.slice(0, 8) || 'MISSING',
    hasClientSecret: !!providerConfig.clientSecret,
    accountsUrl: providerConfig.accountsUrl,
    scopes: providerConfig.oauthScopes,
  });
